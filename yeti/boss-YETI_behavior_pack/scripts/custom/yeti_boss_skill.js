...
const CHILL_AURA_CONFIG = {
    'ytaun:yeti_1':     { radius: 3, amplifier: 0 },
...
function castIceStomp(yeti) {
    try {
        ...
        yeti.dimension.spawnParticle('minecraft:huge_explosion_emitter', loc);
        yeti.dimension.spawnParticle('minecraft:basic_crack_particle', loc);
        ringParticle(yeti.dimension, loc, 1.0, 10, 'minecraft:basic_crack_particle', 0.05);
        ringParticle(yeti.dimension, loc, 2.0, 18, 'minecraft:snowflake_particle');
        ringParticle(yeti.dimension, loc, 3.5, 24, 'minecraft:ice_evaporation_particle');
...
function castFrostNova(yeti) {
    try {
        ...
        for (let r = 1; r <= 5; r++) {
            system.runTimeout(() => {
                try {
                    ringParticle(yeti.dimension, loc, r, 8 + r * 3, 'minecraft:ice_evaporation_particle', 0.2);
...
function castBlizzardFury(yeti) {
    try {
        ...
        yeti.dimension.spawnParticle('minecraft:snowflake_particle', loc);
        yeti.dimension.spawnParticle('minecraft:huge_explosion_emitter', loc);
        ...
        const blizzardTimer = system.runInterval(() => {
            ...
            try {
                ...
                ringParticle(yeti.dimension, yeti.location, 3, 16, 'minecraft:ice_evaporation_particle', 0.3);
                ringParticle(yeti.dimension, yeti.location, 6, 28, 'minecraft:snowflake_particle', 1.0);
...
function healOnKill(yeti) {
    try {
        ...
        ringParticle(yeti.dimension, yeti.location, 1.5, 14, 'minecraft:snowflake_particle', 1.0);
...
function tickFrostRegen(yetis) {
    ...
    for (const yeti of yetis) {
        try {
            ...
            health.setCurrentValue(newValue);
            yeti.dimension.spawnParticle('minecraft:ice_evaporation_particle', yeti.location);
        } catch (e) {
...
world.afterEvents.entityHitEntity.subscribe(event => {
    try {
        ...
        target.dimension.spawnParticle('minecraft:snowflake_particle', target.location);
...
world.afterEvents.entityDie.subscribe(event => {
    ...
    // Người chơi chết dưới tay Yeti → Yeti hồi 15% máu tối đa
