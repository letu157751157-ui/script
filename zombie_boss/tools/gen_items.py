...
def hammer_icon():
    ...
    spikes = layer(lambda d: [d.polygon([P(t, s0), P(t + 1.2 * sgn, s0 + 2.2 * (1 if s0 > 0 else -1)), P(t + 1.6 * sgn, s0)], fill=MAG)
                              for t, sgn in ((5.5, 1), (-5.5, -1)) for s0 in (-8.5, 8.5)], "iron")
    img = outline(stack(handle, grip, rings, pommel, spikes, head, caps, bands, skull, gems))
    rp = img.load()
    for s0 in (-8, 8):
        x, y = P(0, s0); rp[int(x), int(y)] = col("crystal", 3)
    return img
...
def paint(tp, mat, fname, x0, y0, fw, fh, special=None):
    for y in range(fh):
        for x in range(fw):
            ...
            if mat == "wood":
                ...
                if fname != "up" and (x * 7) % 11 == 3 and y < fh * (0.25 + 0.3 * ((x * 13) % 5) / 5):
                    tp[x0 + x, y0 + y] = hx("#6e1812" if y % 3 else "#8a2018"); continue           # máu chảy
            elif mat == "bone":
    ...
    if special == "rune":                                                                  # rune phát sáng
        g1, g2 = C("crystal", 5), C("crystal", 3)
        cxr, cyr = x0 + fw // 2, y0 + fh // 2
        r = min(fw, fh) // 2 - 3
        for i in range(48):
            a_ = i / 48 * 2 * math.pi
            tp[int(cxr + math.cos(a_) * r), int(cyr + math.sin(a_) * r)] = g2
        for k_ in range(3):
            a_ = k_ * 2 * math.pi / 3 - math.pi / 2
            for t_ in range(r):
                tp[int(cxr + math.cos(a_) * t_), int(cyr + math.sin(a_) * t_)] = g1
        tp[cxr, cyr] = C("crystal", 6)
...
BONES = {  # name: (parent, pivot, [(origin, size, mat, extra)])
    "rightitem": (None, [0, 0, 0], [
        ...
        ([-2, 16, -2], [4, 2, 4], "iron", {}),
        ([-2.5, -2.2, -0.5], [1, 1, 1], "bone", {"rotation": [0, 0, 30]}),      # gai xương trên các đốt
        ([1.5, 4.3, -0.5], [1, 1, 1], "bone", {"rotation": [0, 0, -30]}),
        ([-0.5, 10.8, -2.5], [1, 1, 1], "bone", {"rotation": [30, 0, 0]}),
        ([-2.25, 14.5, -2.25], [4.5, 1, 4.5], "iron", {}),                       # vành sắt dưới cổ
    ]),
    "tassel": ("rightitem", [0, -14, 0], [
        ([-0.5, -19, -0.5], [1, 5, 0], "rag", {}), ([0.5, -18, 0], [1, 4, 0], "rag", {}),
        ([-0.25, -20, -0.25], [0.5, 1, 0.5], "bone", {}),
    ]),
    "hammer_head": ("rightitem", [0, 22, 0], [
        ([-5, 18, -4], [10, 9, 8], "flesh", {}),
        ([-3.5, 17.5, -4.5], [1, 10, 9], "iron", {}),
        ([2.5, 17.5, -4.5], [1, 10, 9], "iron", {}),
        ([-8, 17, -4.5], [3, 11, 9], "iron", {"special": "rune", "faces": ("west", "east")}),
        ([5, 17, -4.5], [3, 11, 9], "iron", {"special": "rune", "faces": ("west", "east")}),
        # gai sắt ở 4 góc mỗi đầu bịt
        ([-9, 26, -4], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, 35]}), ([-9, 26, 2.5], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, 35]}),
        ([-9, 16, -4], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, -35]}), ([-9, 16, 2.5], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, -35]}),
        ([7.5, 26, -4], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, -35]}), ([7.5, 26, 2.5], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, -35]}),
        ([7.5, 16, -4], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, 35]}), ([7.5, 16, 2.5], [1.5, 2, 1.5], "iron", {"rotation": [0, 0, 35]}),
        # hàm dưới đầu lâu
        ([-1.5, 18, -5.5], [3, 1.2, 1], "bone", {}), ([-1.5, 18, 4.5], [3, 1.2, 1], "bone", {}),
        # vương miện sắt giữ pha lê
        ([-2, 26.8, -2], [4, 0.8, 4], "iron", {}),
...
for bname, (parent, pivot, cubes) in BONES.items():
    ...
    for origin, size, mat, extra in cubes:
        ...
        for fname, (fx, fy, fw, fh) in {"up": (cu + d, cv, w, d), "down": (cu + d + w, cv, w, d), "east": (cu, cv + d, d, h),
            ...
            sp = extra.get("special") if fname in extra.get("faces", ("north",)) else None
...
anim["animations"]["animation.ytaun_zombieaxe.alive"] = {"loop": True, "bones": {
    ...
    "tassel": {"rotation": ["math.sin(q.life_time * 140) * 10", 0, "math.sin(q.life_time * 110) * 14"]},
}}
